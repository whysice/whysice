...
import { ArrowLeft, Plus, Pill, X, ExternalLink } from 'lucide-react'
...
export default function TreatmentsPage() {
  ...
  useEffect(() => {
    ...
    getTreatmentLogs(activeDogId).then(setTreatments)
  }, [activeDogId])

  // Auto-fill name when wiki med selected
  useEffect(() => {
    if (medicationId) {
      const med = wikiMeds.find(m => m.id === medicationId)
      if (med) {
        setTreatmentName(med.brand_names?.[0] || med.name)
      }
    }
  }, [medicationId, wikiMeds])
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!error) {
      const updated = await getTreatmentLogs(activeDogId)
    ...
    <div className="max-w-3xl mx-auto px-4 py-8">
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="card mb-8 border-tanzanite-200">
          <h2 className="font-semibold text-tanzanite-800 mb-4">New treatment</h2>

          {/* Link to wiki medication */}
          <div className="mb-4">
            ...
            <select
              id="wiki-med" value={medicationId} onChange={e => setMedicationId(e.target.value)}
            ...
            >
              ...
              {wikiMeds.map(med => (
                <option key={med.id} value={med.id}>
                  {med.brand_names?.[0] || med.name} ({med.name})
                </option>
              ))}
            </select>
          </div>
          ...
          <div className="grid sm:grid-cols-2 gap-4 mb-4">
            <div>
              ...
              <input
                id="treatment-name" type="text" value={treatmentName}
            ...
            <div>
              ...
              <input
                id="dosage" type="text" value={dosage} onChange={e => setDosage(e.target.value)}
          ...
          <div className="grid sm:grid-cols-3 gap-4 mb-4">
            <div>
              ...
              <input
                id="date-started" type="date" value={dateStarted} onChange={e => setDateStarted(e.target.value)}
                className="w-full px-3 py-2 rounded-lg border border-tanzanite-100 text-sm focus:border-tanzanite-500 focus:outline-none focus:ring-1 focus:ring-tanzanite-200"
                required
              />
            </div>
            <div>
              ...
              <input
                id="date-ended" type="date" value={dateEnded} onChange={e => setDateEnded(e.target.value)}
                className="w-full px-3 py-2 rounded-lg border border-tanzanite-100 text-sm focus:border-tanzanite-500 focus:outline-none focus:ring-1 focus:ring-tanzanite-200"
              />
            </div>
            <div>
              ...
              <select
                id="frequency" value={frequency} onChange={e => setFrequency(e.target.value)}
                className="w-full px-3 py-2 rounded-lg border border-tanzanite-100 text-sm focus:border-tanzanite-500 focus:outline-none focus:ring-1 focus:ring-tanzanite-200"
              >
          ...
          <div className="mb-4">
            ...
            <input
              id="effectiveness" type="range" min="0" max="5" value={effectiveness}
              onChange={e => setEffectiveness(Number(e.target.value))}
              className="w-full accent-tanzanite-500"
            />
          ...
          <div className="mb-4">
            ...
            <textarea
              id="side-effects" value={sideEffects} onChange={e => setSideEffects(e.target.value)}
          ...
          <div className="mb-4">
            ...
            <textarea
              id="treatment-notes" value={notes} onChange={e => setNotes(e.target.value)}
...
function TreatmentCard({ treatment: t, isPast }: { treatment: TreatmentLog; isPast?: boolean }) {
  return (
    <div className={`card py-4 ${isPast ? 'opacity-70' : ''}`}>
      <div className="flex items-start justify-between gap-3 mb-2">
        <div>
          ...
          <div className="flex flex-wrap gap-x-3 text-xs text-slate mt-0.5">
            ...
            <span>
              {formatDate(t.date_started)}
              {t.date_ended ? ` - ${formatDate(t.date_ended)}` : ' - ongoing'}
            </span>
          </div>
        </div>
      ...
      {t.side_effects_observed && (
        <p className="text-xs text-red-500 mt-1">Side effects: {t.side_effects_observed}</p>
      )}
      {t.notes && (
        <p className="text-xs text-slate mt-1">{t.notes}</p>
      )}
      {t.medications && (
        <Link
